...
public static class EntityAssembler
{
    ...
    public static Plant BuildPlant(PlantData data)
    {
        ...
        var plant = new Plant(data.Id, data.DisplayName);
        int count = AttachComponents(plant, data.Components, data.Id);

        GD.Print($"[装配] 植物 {plant.Id}（{plant.DisplayName}）组件 {count} 个");
        return plant;
    }

    /// <summary>
    /// 按配置装配一只僵尸，摆在 position（区域像素空间），站在 lawn 上。
    /// position 指的是僵尸的嘴，不是身体中心。
    /// </summary>
    public static Zombie BuildZombie(ZombieData data, Vector2 position, TilesData lawn)
    {
        if (data == null)
        {
            return null;
        ...
        // 实体自己的 Id 只是个给人看的标签；僵尸真正的编号是数据里的整数
        ...
        GD.Print($"[装配] 僵尸 {zombie.Id}（{zombie.DisplayName}）血量 {data.Hp}，组件 {count} 个");
        return zombie;
    }
    ...
    ///
    /// `configId` 是"哪份配置配出了这些组件"：自己身上的是这个实体，
    /// 将来从别处（buff、装备之类的）挂过来的则是**配它的那一个**，
    /// 组件找数据时用它，而不是用现在挂着的这个。
    /// </summary>
